...
Usage:
    ...
    python train.py --mode learned_hybrid
    python train.py --mode learned_loss_weighted
...
Modes:
    ...
    learned_hybrid        - mostly uniform + learned high-loss tilt + hard floor
    learned_loss_weighted - learned p(t;theta) matched to high-loss timesteps
...
class LearnedSampler:
    ...
    def _sampling_probs(self):
        ...
        if self.prob_floor > 0:
            floor_mass = self.T * self.prob_floor
            if floor_mass >= 1.0:
                raise ValueError("prob_floor is too large for the number of timesteps")
            p = self.prob_floor + (1.0 - floor_mass) * p

        if self.uniform_mix > 0:
            uniform = torch.full_like(p, 1.0 / self.T)
            p = self.uniform_mix * uniform + (1.0 - self.uniform_mix) * p

        return p / p.sum()

    def sample(self, B):
        p = self._sampling_probs().detach()
        return torch.multinomial(p, num_samples=B, replacement=True).to(self.device)

    def weights(self, t):
        p = self._sampling_probs()
        uniform = 1.0 / self.T
        return (uniform / (p[t] + 1e-8)).detach()
    ...
    def update(self, per_t_loss_np):
        ...
        p = self._sampling_probs()
        if self.objective == "expected_loss":
            theta_loss = (p * per_t_loss_tensor).sum()
        elif self.objective == "loss_weighted":
            if self.loss_ema is None:
                self.loss_ema = per_t_loss_tensor.detach()
            else:
                beta = config.THETA_LOSS_EMA
                self.loss_ema = beta * self.loss_ema + (1.0 - beta) * per_t_loss_tensor.detach()
            target = self.loss_ema.clamp_min(1e-8).pow(config.THETA_LOSS_POWER)
            target = target / target.sum()
            theta_loss = -(target.detach() * (p + 1e-8).log()).sum()
        else:
            raise ValueError(f"Unknown theta objective: {self.objective}")

        if self.use_constraints:
            lag_val, cov_viol, ent_viol = self.lagrangian.loss(
                self.theta_dist, theta_loss
            )
        else:
            lag_val = theta_loss
...
def train(mode, subset_start=60, subset_end=99):
    ...
    elif mode == "learned_loss_weighted":
        sampler    = LearnedSampler(
            config.T, device, use_constraints=True, objective="loss_weighted"
        )
        is_learned = True

    elif mode == "learned_hybrid":
        sampler    = LearnedSampler(
            config.T,
            device,
            use_constraints=True,
            objective="loss_weighted",
            uniform_mix=config.HYBRID_UNIFORM_MIX,
            prob_floor=config.HYBRID_PROB_FLOOR,
        )
        is_learned = True
    ...
    for step in range(1, config.TOTAL_STEPS + 1):
        ...
        if step % config.THETA_UPDATE_EVERY == 0:
            per_t_loss = compute_per_timestep_loss(
                model, loader_iter, alpha_bar, device,
                n_batches=config.THETA_ESTIMATE_BATCHES
            )
